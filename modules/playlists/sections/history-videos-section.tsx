...
import { HistoryVideoType } from '@/types';
...
export const HistoryVideosSection = () => {
	...
	const [videoList, setVideoList] = useState<HistoryVideoType>({
		historyVideosWithLimit: [],
		hasNextPage: false,
	});

	const getHistoryVideoList = async (currentOffset = 0, initialLoad = true) => {
		const count = currentOffset * 5;
		setIsLoading(true);
		const res = await fetch(`/api/history-videos?offset=${count}&userId=${user?.id}`, {
			method: 'GET',
		});
		setIsLoading(false);
		...
		setVideoList((prev) => {
			...
			return {
				HistoryVideosWithLimit: [...prev.historyVideosWithLimit, ...response.historyVideosWithLimit],
	...
	const fetchNextVideoPage = () => {
		...
		getHistoryVideoList(offset, false);
	};
			...
			{!isLoading ? (
				<>
					<div className="flex flex-col gap-4 gap-y-10 md:hidden">
						{videoList.historyVideosWithLimit.map((video) => (
